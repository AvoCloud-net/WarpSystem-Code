...
public class ParticleOptionsColor implements ItemListener {
    private final ParticleOptions instance;

    public ParticleOptionsColor(ParticleOptions instance) {
    ...
    public void onClick(HotbarGUI gui, ItemComponent ic, Player player, ClickType clickType) {
        ...
        instance.getMain().getAnimation().update();
    ...
    public void onHover(HotbarGUI gui, ItemComponent old, ItemComponent current, Player player) {
        if (instance.getPart().getParticle().isColorable()) {
            MessageAPI.sendActionBar(instance.getPlayer(), AnimationHotBarEditor.PREVIOUS_NEXT(Lang.get("Particle_Effect")), WarpSystem.getInstance(), Integer.MAX_VALUE);
        } else {
