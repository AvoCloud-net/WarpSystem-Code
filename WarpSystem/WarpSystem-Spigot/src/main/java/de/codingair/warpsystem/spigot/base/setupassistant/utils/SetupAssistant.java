...
public class SetupAssistant {
    ...
    protected void process(String arg, String warning) {
        ...
        if (l == null) {
            //menu
            if (PluginVersion.getUpcoming() != null && filter != null && filter.ordinal() >= PluginVersion.getUpcoming().ordinal() && SetupAssistantManager.getInstance().getFunction(filter).isEmpty()) {
            ...
            l.stream().skip(page * 6).limit(6).forEach(value -> {
                ...
                if (value.getDescription().isEmpty()) info.setColor(net.md_5.bungee.api.ChatColor.YELLOW);
                    ...
                    info.setItalic(true);
                    if (Version.after(15)) info.setColor(net.md_5.bungee.api.ChatColor.of(new Color(193, 252, 0)));
