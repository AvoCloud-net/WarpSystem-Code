...
public abstract class PlayerWarpHandler implements Manager {
    ...
    public boolean load(boolean loader, DataMask playerWarps, DataMask config) {
        if (!loader) Core.getPlugin().log("  > Loading PlayerWarps");

        inactiveTime = convertFromTimeFormat(config.getString("WarpSystem.PlayerWarps.Inactive.Time_After_Expiration"), 86400000L);
        ...
        JSONArray l = playerWarps.getList("PlayerWarps");
        int size = 0;
        for (Object o : l) {
            if (o instanceof Map<?, ?>) {
                Map<?, ?> map = (Map<?, ?>) o;
        ...
        interactWithServers(server -> Core.getPlugin().dataHandler().send(packet, server, Direction.DOWN));

        runScheduler();
        if (!loader) Core.getPlugin().log("    ...got " + size + " PlayerWarp(s)");
        return true;
    }

    public void save(boolean saver, DataMask playerWarps) {
        if (!saver) Core.getPlugin().log("  > Saving PlayerWarps...");
        ...
        playerWarps.put("PlayerWarps", l);
        if (!saver) Core.getPlugin().log("    ...saved " + l.size() + " PlayerWarp(s)");
    }

    public void runScheduler() {
        if (task != null) return;
        task = Core.getPlugin().schedule(() -> {
    ...
    public void interactWithServers(ServerInteraction runnable) {
        this.activeServers.forEach(runnable::interact);
    }
    ...
    public boolean isActive(Server info) {
        return this.activeServers.contains(info);
    }

    public boolean setActive(Server info, boolean active) {
        if (active) {
    ...
    public boolean setTimeDependent(Server info, boolean timeDependent) {
        if (timeDependent) {
